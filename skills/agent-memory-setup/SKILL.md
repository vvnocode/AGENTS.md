...
| 手改 `codex-*.md` 或它的索引行 | 索引段每次整段重生成；改了正文句子等于换了一条，下次同步会按新句子另建文件 | 要修正就删掉该文件另写一条正常记忆；索引段之外的行不会被动 |
...
- **粒度与格式**：Codex 的每个列表项就是一条记忆，一条一文件平铺在 `.memory/` 根下，frontmatter 只有 `name`、`description`、`metadata.type` 加溯源字段 `source: codex`、`thread_id`、`observed_at`；正文是 Codex 原句加一行来源。**只收做事方式**：`Preference signals` 与 `Failures and how to do differently` → `feedback`；`Reusable knowledge` 是事实结论，按「记忆记做事方式、结论进 wiki / docs」的分工**不落地**（原文仍在 Codex 自己的记忆里，有新增时在同步输出里报跳过条数）。`References` 与 `rollout_path` 丢弃。
- **文件名** `codex-<句子开头 12 字>-<hash6>.md`，`hash6` 是句子归一后的 sha256 前 6 位：文件名能一眼看出内容、与手写条目同风格，去重仍靠哈希——同一句子跨来源、跨线程只一条，Codex 重排列表项不产生重复文件，已存在的文件不重写。既有条目按**归一句子**认领：命名方案变了就改名，不会留下两份（旧的 `codex-<task_group>-<hash10>` 会在下次同步时自动改名）。线程有 raw 块时它的会话摘要整份不取（raw 块是 Codex 对同一 rollout 做的记忆提取，摘要是叙事复述，换个说法的同一事实按句子去重抓不到），没有 raw 块的线程才用摘要。不删除来源已消失的条目。
...
- **自动提交**：同步后若仓内 `.memory` **已入库**，用 pathspec 偏提交 `.memory`（`git commit -- .memory`），顺带把上次会话遗留的记忆改动一并提交；偏提交不会带走 index 里别人已暂存的改动。未入库（如规则仓自身把 `.memory` gitignore）不提交；合并 / cherry-pick / 变基进行中、HEAD 游离时跳过并告警；`RULES_NO_MEMORY_COMMIT=1` 整个关掉。
